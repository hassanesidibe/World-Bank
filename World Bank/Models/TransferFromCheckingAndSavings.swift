//
//  TransferFromCheckingAndSavings.swift
//  World Bank
//
                ...
                db.collection(bankAccountConstants.collectionName)
                    ...
                    .getDocuments { querySnapshot, error in
                        if let err = error {
                            ...
                            if let accountDocument = querySnapshot!.documents.first {
                                let accountData = accountDocument.data()
                                let recipientAccountReference = self.db.collection(bankAccountConstants.collectionName).document(recipientEmail)
                                
                                //Deposit to recipient's checking account
                                let recipientCheckingBalanceString = accountData[bankAccountConstants.checkingBalanceField] as! String
                                
                                if let recipientCurrentCheckingBalanceDouble = Double(recipientCheckingBalanceString) {
                                    let newBalance = (recipientCurrentCheckingBalanceDouble + transferAmount)
                                    recipientAccountReference.updateData([bankAccountConstants.checkingBalanceField : "\(newBalance)"])
